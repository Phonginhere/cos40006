...
class UserPersonaLoader:
    ...
    def load(self):
        ...
        # Create sample and uploaded persona directories if they don't exist
        os.makedirs(utils.SAMPLE_PERSONA_DIR, exist_ok=True)
        ...
        # Step 1: Check if we have any personas at all
        if not self.sample_personas and not self.uploaded_personas:
            if not self.no_logging:
                print("⚠️ No personas found in either sample or uploaded directories.")
            self.personas = []
            return

        # Step 1: Ensure filename uniqueness
        sample_files = set(os.listdir(utils.SAMPLE_PERSONA_DIR)) if os.path.exists(utils.SAMPLE_PERSONA_DIR) else set()
        uploaded_files = set(os.listdir(utils.UPLOADED_PERSONA_DIR)) if os.path.exists(utils.UPLOADED_PERSONA_DIR) else set()
...
class Utils:
    ...
    def set_model(self, model_name: str):
        """Update the current LLM model and reinitialize all dependent paths."""
        self.CURRENT_LLM = model_name
        # Reinitialize all paths that depend on the model
        self._init_results_paths()
        
        # Log the change
        print(f"Model updated to: {self.CURRENT_LLM}")
        print(f"Results directory updated to: {self.ROOT_RESULTS_DIR}")
    ...
    def load_api_key(self, path: str = "api_key.txt") -> str:
        """Load API key from multiple possible locations"""
        # If API key is already set, just return it
        if hasattr(self, 'api_key') and self.api_key and len(self.api_key) > 0:
            return self.api_key
            
        # List of possible locations to check for api_key.txt
        possible_paths = [
            path,  # Current working directory (original behavior)
            os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), path),  # Project root
            os.path.abspath(path)  # Absolute path if provided
        ]
        ...
        for p in possible_paths:
            try:
                with open(p, "r") as file:
                    key = file.read().strip()
                    if not key:
                        continue  # Empty file, try next path
        ...
        # If we get here, we didn't find a valid key file
