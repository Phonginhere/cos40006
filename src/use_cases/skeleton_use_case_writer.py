...
from use_cases.use_case_loader import load_use_case_type_config
from utils import USE_CASE_DIR
...
def write_use_case_skeletons(persona_loader: UserPersonaLoader, seed: int = None) -> None:
    ...
    personas = [p for p in persona_loader.get_personas() if p.id != "P-003"]
    ...
    for idx, uc_type in enumerate(uc_types, 1):
        ...
        chosen = []
        for g in u_groups:
            chosen.extend(sample_personas_by_group(
                by_group[g], pick_two_prob=rule.get("pickTwoPersonasRate", 0.3)))
    ...
    for uc in use_cases:
        (Path(USE_CASE_DIR) / f"{uc['id']}.json").write_text(json.dumps(uc, indent=2, ensure_ascii=False))
