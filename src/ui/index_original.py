...
@st.cache_data
def get_available_models(persona_combination=None):
    """Get available model directories for a specific persona combination, or all models if no persona is specified"""
    if not persona_combination:
        # Return a flat list of all models across all persona combinations
        results_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "results")
        all_models = []
        try:
            if os.path.exists(results_dir):
                ...
                for persona_dir in persona_dirs:
                    ...
                    models = [os.path.join(persona_dir, d) for d in os.listdir(persona_path) 
        ...
        # Return models for a specific persona combination
...
def run_pipeline(api_key, model_id=None):
    """Run the main.py pipeline in a separate process and capture its output"""
    # Verify the API key exists
    if not api_key or len(api_key.strip()) == 0:
        st.session_state.log_content += "❌ No API key provided. Please enter or upload an API key first.\n"
        return False
    ...
    try:
        # Use the wrapper script instead of main.py directly
        pipeline_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "run_pipeline.py")
        
        # Set working directory to the src directory to ensure relative paths work
        working_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        
        # Prepare command with model argument if provided
...
with st.sidebar:
    ...
    if api_key_method == "Text Input":
        ...
        api_key = st.text_input(
            ...
            type="password", 
            key="api_key_input",
            help="Enter your API key here. The key is saved automatically as you type."
        )
        
        # Auto-save the API key as the user types
        if api_key != existing_api_key and api_key.strip():
            # Import the update_api_key function if available
            try:
                from pipeline.api_utils import update_api_key
                update_api_key(api_key)
                st.success("✅ API key saved automatically!")
            except ImportError:
                # Fall back to the old method if api_utils is not available
                if save_api_key_to_file(api_key):
                    st.success("✅ API key saved automatically!")
                
        # Display info about the API key status
        if api_key.strip():
            st.info("API key is active and ready to use.")
                
        # Add a "Test API Key" button in the future
    else:
        ...
        if existing_api_key:
            st.info("An API key is already saved. Upload a new file to replace it.")
            
        # Use existing key as the default
        api_key = existing_api_key
            
        # Allow user to upload a new key file
        api_key_file = st.file_uploader("Upload API Key File", type=['txt'], help="Upload a text file containing just your API key.")
        
        if api_key_file:
            new_api_key = api_key_file.getvalue().decode("utf-8").strip()
            if new_api_key:
                if save_api_key_to_file(new_api_key):
                    st.success("✅ API key loaded and saved successfully!")
                    # Update the current api_key to use the new one
                    api_key = new_api_key
                else:
    ...
    if st.button("Process Inputs", type="primary", disabled=st.session_state.process_running):
        if not api_key:
            ...
            with st.spinner("Processing..."):
                try:
                    st.session_state.log_content += "Starting process_with_api...\n"
                    ...
                    st.session_state.log_content += f"Traceback: {error_traceback}\n"
