...
# Function to get available systems
@st.cache_data
def get_available_systems():
    """Get available system directories from the systems directory"""
    systems_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "results")
    systems = []
    try:
        if os.path.exists(systems_dir):
            systems = [d for d in os.listdir(systems_dir) if os.path.isdir(os.path.join(systems_dir, d))]
    except Exception as e:
...
@st.cache_data
def get_available_persona_combinations(system):
    """Get available persona combination directories from the results directory"""
    results_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "results", system)
...
def get_models_for_persona(persona_combination):
    ...
    persona_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "results", persona_combination)
...
def get_conflict_files(persona_combination, model_name, conflict_type, functional_type):
    ...
    try:
        ...
        conflict_dir = "conflicts_within_one_group" if conflict_type == "within" else "conflicts_across_two_groups"
        
        # Base path to conflicts directory for the specified model
        base_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
            "results", persona_combination, model_name, conflict_dir
        )
...
def load_conflict_data(persona_combination, model_name, conflict_type, functional_type):
    ...
    try:
        ...
        if all_conflicts:
            ...
            conflict_type_label = "within-group" if conflict_type == "within" else "across-group"
            return f"No {conflict_type_label} {functional_type} conflict data found for model: {persona_combination}/{model_name}"
    except Exception as e:
...
# Get available systems from the results directory
...
# Get available persona combinations for the selected system
    if selected_system:
        available_persona_combinations = get_available_persona_combinations(selected_system)
    ...
    st.warning("No systems found in the results directory. Please run the pipeline first.")
...
if selected_persona_combination and selected_model_name:
    ...
    with col_results_2:
        ...
        functional_cross_files = get_conflict_files(selected_persona_combination, selected_model_name, "cross", "functional")
        non_functional_cross_files = get_conflict_files(selected_persona_combination, selected_model_name, "cross", "non_functional")
