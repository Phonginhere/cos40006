...
LLM_RESPONSE_LANGUAGE_PROFICIENCY_LEVEL_PATH = os.path.join("data", "llm_response_language_proficiency_level.txt")

# Constants base on the system name
ROOT_DATA_DIR = os.path.join("data", SYSTEM_NAME)

SYSTEM_SUMMARY_PATH = os.path.join(ROOT_DATA_DIR, "system_summary.txt")
USER_GROUPS_DIR = os.path.join(ROOT_DATA_DIR, "user_groups")
PILLARS_DIR = os.path.join(ROOT_DATA_DIR, "pillars")
...
FUNCTIONAL_USER_STORY_CLUSTERING_TECHNIQUE_DESCRIPTION_PATH = os.path.join(ROOT_DATA_DIR, "user_story_rules", "functional_user_story_clustering_technique_guidelines.txt")

NON_FUNCTIONAL_USER_STORY_CONFLICT_TECHNIQUE_DESCRIPTION_PATH = os.path.join(ROOT_DATA_DIR, "user_story_conflict_rules", "non_functional_user_story_conflict_summary.txt")
FUNCTIONAL_USER_STORY_CONFLICT_TECHNIQUE_DESCRIPTION_PATH = os.path.join(ROOT_DATA_DIR, "user_story_conflict_rules", "functional_user_story_conflict_summary.txt")

# ==============================================================================================
# LLM RESPONSE LANGUAGE PROFICIENCY LEVEL LOADER
def load_llm_response_language_proficiency_level() -> str:
    """Loads the LLM response language proficiency level from a text file."""
    try:
        with open(LLM_RESPONSE_LANGUAGE_PROFICIENCY_LEVEL_PATH, "r", encoding="utf-8") as f:
            return f.read().strip()
    except FileNotFoundError:
        raise FileNotFoundError(f"❌ Missing LLM response language proficiency level file at: {LLM_RESPONSE_LANGUAGE_PROFICIENCY_LEVEL_PATH}")

# ==================================================================================================
# ALFRED SYSTEM SUMMARY & PILLARS LOADER

def load_pillar_keys() -> dict:
    """Dynamically load mapping from pillar name to filename."""
    mapping = {}
    for filename in os.listdir(PILLARS_DIR):
        if not filename.endswith(".json"):
            continue
        full_path = os.path.join(PILLARS_DIR, filename)
        try:
...
def load_all_pillar_descriptions(system_name: str = SYSTEM_NAME) -> str:
    """Return a formatted string of all pillar names and descriptions grouped by functional and cross-functional."""
    filename_map = load_pillar_keys()
    functional_pillars = []
    cross_functional = {}
...
def load_system_summary() -> str:
    """Loads the ALFRED system summary from a text file."""
    try:
        with open(SYSTEM_SUMMARY_PATH, "r", encoding="utf-8") as f:
            return f.read().strip()
    except FileNotFoundError:
        raise FileNotFoundError(f"❌ Missing ALFRED summary file at: {SYSTEM_SUMMARY_PATH}")

def load_system_context(system_name: str = SYSTEM_NAME) -> str:
    """Loads the ALFRED system summary and appends pillar structure details."""
    try:
        with open(SYSTEM_SUMMARY_PATH, "r", encoding="utf-8") as f:
    ...
    pillar_descriptions = load_all_pillar_descriptions(system_name)
...
def load_user_group_keys() -> dict:
    """
    Dynamically reads all JSON files in USER_GROUPS_DIR and constructs a dict:
    { group_name (from 'name' field): file_key (e.g., user_group_1) }
    """
    keys = {}
    for filename in os.listdir(USER_GROUPS_DIR):
        if filename.endswith(".json"):
            file_key = filename.replace(".json", "")
            path = os.path.join(USER_GROUPS_DIR, filename)
            try:
...
def get_user_groups() -> list:
    """
    Returns a list of user group names, derived from the keys in USER_GROUPS_DIR.
    """
    return list(load_user_group_keys().keys())

def load_user_group_description(group: str) -> str:
    """Loads the user group guidelines from a JSON file based on the user group."""
    guidelines_files = {
        value: os.path.join(USER_GROUPS_DIR, f"{value}.json")
...
def load_non_functional_user_story_cluster_summary(pillar: str) -> list:
    """Load non-functional user story clusters (clusterList only) for the given pillar."""
    filename_map = load_pillar_keys()
...
def load_non_functional_user_story_conflict_technique_description() -> str:
    try:
        with open(NON_FUNCTIONAL_USER_STORY_CONFLICT_TECHNIQUE_DESCRIPTION_PATH, "r", encoding="utf-8") as f:
            return f.read().strip()
    except FileNotFoundError:
        raise FileNotFoundError(f"❌ Missing non-functional user story conflict summary file at: {NON_FUNCTIONAL_USER_STORY_CONFLICT_TECHNIQUE_DESCRIPTION_PATH}")
    
def load_functional_user_story_conflict_technique_description() -> str:
    try:
        with open(FUNCTIONAL_USER_STORY_CONFLICT_TECHNIQUE_DESCRIPTION_PATH, "r", encoding="utf-8") as f:
            return f.read().strip()
    except FileNotFoundError:
        raise FileNotFoundError(f"❌ Missing non-functional user story conflict summary file at: {FUNCTIONAL_USER_STORY_CONFLICT_TECHNIQUE_DESCRIPTION_PATH}")

def load_functional_user_story_clustering_technique_description() -> str:
    try:
        with open(FUNCTIONAL_USER_STORY_CLUSTERING_TECHNIQUE_DESCRIPTION_PATH, "r", encoding="utf-8") as f:
            return f.read().strip()
    except FileNotFoundError:
        raise FileNotFoundError(f"❌ Missing functional user story clustering technique file at: {FUNCTIONAL_USER_STORY_CLUSTERING_TECHNIQUE_DESCRIPTION_PATH}")
...
class UserPersona:
    ...
    def classify_user_group(self) -> str:
        ...
        group_summaries = {
            name: load_user_group_description(user_group_keys[name])
        ...
        prompt = f"""
You are a classifier for the following system: {load_system_context()}.
